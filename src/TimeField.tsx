...
import { from12h, smart12hText, to12h, toggleAmPm } from "./time";

type Props = {
  value24: string; // "HH:MM"
...
export default function TimeField({ value24, onChange24, ariaLabel }: Props) {
  ...
  function commit(nextRaw: string) {
    ...
    if (!maybeText) {
      const direct = from12h(nextRaw, parsed.ampm);
      if (direct) onChange24(direct);
      return;
    }
    const maybe24 = from12h(maybeText, parsed.ampm);
